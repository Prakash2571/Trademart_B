...
# Store classification lives on /api/shopify/status (refined with Shopify's real
# isDevelopmentStore flag). There is deliberately no second endpoint for it.
expect_status 200 GET /api/shopify/status              "GET /api/shopify/status"
...
# The lock and the Shopify breaker are reported by /automation/status - not by a
# separate endpoint, so there is one authority for "can I apply right now?".
expect_status 200 GET /api/automation/status  "GET /api/automation/status"
...
if [[ "$WITH_WRITES" -eq 0 ]]; then
  ...
  if [[ "$created" == "201" || "$created" == "207" ]]; then
    ...
    if [[ -n "${PRODUCT_ID:-}" ]]; then
      ...
      if [[ "$(req GET "/api/shopify/products/${encoded}/publications")" == "200" ]]; then

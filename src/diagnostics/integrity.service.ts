...
import { resolveOnlineStorePublication } from '../shopify/publication.service';
import { PRODUCTS_PUBLICATION_AUDIT_QUERY } from '../shopify/graphql/publication.queries';
...
async function checkOrphanedCosts(
  ...
  const records = (await CostRecordModel.find({
    ...
    .select('shopifyProductId shopifyVariantId amount currency')

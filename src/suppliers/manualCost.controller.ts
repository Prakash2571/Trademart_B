...
manualCostRouter.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Read the existing record first so the audit entry can record what the cost
    // WAS. A cost override is a hand-entered number that exists nowhere else, so
    // "it used to be 4.20" has to be recoverable after someone changes it.

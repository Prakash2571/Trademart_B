...
export interface InventorySetResult {
  ...
  name: string;
  /** The quantity BEFORE the change, so the write is reversible from the audit. */
 ...
 *
 * Returns quantity: null rather than throwing when it cannot be read - the caller
 * decides what an unknown starting point means (currently: require explicit
 * confirmation, because a change of unknown size cannot be capped). A missing
 * read_inventory scope must not block a write that write_inventory would allow.
 */
export async function getCurrentInventoryLevel(
  ...
  try {
    ...
    const match = (level?.quantities ?? []).find((entry) => entry.name === name) ?? null;
 ...
 *
 * Reads the current value FIRST, which is what makes three guarantees possible at
 * once: a stale overwrite can be detected, an oversized change can be capped
 * server-side, and the audit entry can record the previous quantity so the change
 * is reversible. Stock is the field an operator most often changes while looking
 * at a page that is minutes old, so one extra cheap read is worth it.
 */
export async function setInventoryQuantity(
  ...
  // A confirmed large change gets its own log line - this is the write most
  // likely to be regretted, so it should be easy to find afterwards.
